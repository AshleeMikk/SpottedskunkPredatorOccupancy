...
###### JAGS model
Below we have the JAGS model with the effect of stand size on species A and stand age as a binomial Young/Old indicator
```{r}
...
zB <- ifelse(yB > 0, 1, NA)
nSurv <- ncol(DHA)
LyRu.SpGr_data.Age <- list(yA = yA, yB = yB, n = nSurv, nSites = nSites,
...
LyRu_SpGr.p.Age.out <- jags(LyRu.SpGr_data.Age, NULL, wanted,
                      model="CoOccurance_Age.txt",
                      n.chains=4, n.adapt=2000, n.iter=400000,
                      n.burnin = 30000,n.thin = 100,DIC=FALSE,
                      parallel=TRUE)

LyRu.SpGR.p.cov_output <- mcmcOutput(LyRu_SpGr.p.cov.out)
summary(LyRu.SpGR.p.cov_output)

diagPlot(LyRu.SpGR.p.cov_output, max=5)
postPlot(LyRu.SpGR.p.cov_output)
postPlot(LyRu.SpGR.p.cov_output, "psiB", xlim=c(0.025, 0.95))

plot(LyRu.SpGR.p.cov_output, "psiB", xlim=c(0.1, 1))
plot(LyRu.SpGR.p.cov_output, "aSize")
